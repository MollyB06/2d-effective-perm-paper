### fixed time dependent homogenised membrane code - hard copy fenicsx functions as vectors and preallocate memory for speed up
...
def time_sim_effective(eps,n_chunk=20,num_steps=48, final_time=12):
    ...
    np.savetxt('newhom_flux_above_array_long_{}'.format(run), hom_flux_above_array_long)
    np.savetxt('newhom_flux_below_array_long_{}'.format(run), hom_flux_below_array_long)
...
def time_sim_full(eps,n_chunk=20,num_steps=48, final_time=12):
    ...
    ###save as txt files to call into plotting code
    np.savetxt('times_list_long_{}'.format(run),times_list_roundlongarray)
    np.savetxt('newfull_flux_above_array_long_{}'.format(run), full_flux_above_array_long)
    np.savetxt('newfull_flux_below_array_long_{}'.format(run), full_flux_below_array_long)
...
def time_sim_em(eps,n_chunk=20,num_steps=12, final_time=3):
    ...
    times_list_roundshortarray = np.array([round(i*delt,3) for i in range(num_steps)])
    ...
    for i in range(num_steps):
        ...
        du_EM_1_sol.x.array[:] = eps/(L+chi)*(u1_EM_vis_array - u2_EM_vis_array) -EMh_val + EMf_val
        du_EM_2_sol.x.array[:] = eps/(L+chi)*(u1_EM_vis_array - u2_EM_vis_array) +EMh_val + EMf_val
    ...
    np.savetxt('newEM_flux_above_array_{}'.format(run), EM_flux_above_array)
    np.savetxt('newEM_flux_below_array_{}'.format(run), EM_flux_below_array)
