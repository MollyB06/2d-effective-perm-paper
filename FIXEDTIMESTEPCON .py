###new time step convergence code fixed 26_08
...
def effective_sim(delt, final_time = 2.55):
    ...
    l_hom= u_1_n*v1*dx_hom(bottom_sub_marker) + u_2_n*v2*dx_hom(top_sub_marker) -(Constant(mesh_hom, PETSc.ScalarType((2*eps*L)/(L + chi)))*jmp_v*even+ Constant(mesh_hom, PETSc.ScalarType((2*eps)))*plus_v*odd)*dS_hom(interface_marker)
...
TCerror_below_num_v_full_2_5_array = np.array(er_num_v_full_below_2_5)
delt_array = np.array(delt_list)

np.savetxt(results_dir /'TCfull_flux_above_array',TCfull_flux_above_array)
np.savetxt(results_dir /'TCfull_flux_below_array',TCfull_flux_below_array)
np.savetxt(results_dir /'TChom_flux_above_array',TChom_flux_above_array)
np.savetxt(results_dir /'TChom_flux_below_array',TChom_flux_below_array)
np.savetxt(results_dir /'TCerror_above_num_v_full_2_5_array',TCerror_above_num_v_full_2_5_array)
np.savetxt(results_dir /'TCerror_below_num_v_full_2_5_array',TCerror_below_num_v_full_2_5_array)
np.savetxt(results_dir /"delt_array", delt_array)