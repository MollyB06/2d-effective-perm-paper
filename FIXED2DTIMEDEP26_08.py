### fixed time dependent homogenised membrane code - hard copy fenicsx functions as vectors and preallocate memory for speed up fix factor and sign errors 26_08
...
results_dir = Path("/home/user/Documents/FIXED2DTIMEDEP/results")
...
def time_sim_effective(eps,n_chunk=20,num_steps=48, final_time=12):
    ...
    ####also need to initialised [2]?
    H_array[2]=Hm1.x.array.copy()
    F_array[2]=Hm2.x.array.copy()
    ...
    l_hom= u_1_n*v1*dx_hom(bottom_sub_marker) + u_2_n*v2*dx_hom(top_sub_marker) -(Constant(mesh_hom, PETSc.ScalarType((2*L*eps)/(L + chi)))*jmp_v*even +Constant(mesh_hom, PETSc.ScalarType((2*eps)))*plus_v*odd)*dS_hom(interface_marker)
    ...
    u_vis.x.array[:] = u_1_vis.x.array + u_2_vis.x.array
    ...
    times_list_roundlongarray = np.array([round(i*delt,3) for i in range(1,num_steps+1)])
    ...
    for i in range(num_steps):
        print(n,t)
        # print(assemble_scalar(form(-(Constant(mesh_hom, PETSc.ScalarType((2*L*eps)/(L + chi)))*jmp_v*even - Constant(mesh_hom, PETSc.ScalarType((2*eps)))*plus_v*odd)*dS_hom(interface_marker))))
        ...
        du_1_sol.x.array[:] = eps/(L+chi)*(u_2_vis.x.array - u_1_vis.x.array)-2*eps*(1/delt)*odd_val +(2*eps*L)/(L+chi)*(1/delt)*even_val
        du_2_sol.x.array[:] = eps/(L+chi)*(u_2_vis.x.array - u_1_vis.x.array)+2*eps*(1/delt)*odd_val +(2*eps*L)/(L+chi)*(1/delt)*even_val
        ...
        hom_flux_above_array_long[i]=(average_flux_above_hom)
        hom_flux_below_array_long[i]=(average_flux_below_hom)
    ...
    np.savetxt(results_dir/'times_list_long_{}'.format(run),times_list_roundlongarray)
    np.savetxt(results_dir/'newhom_flux_above_array_long_{}'.format(run), hom_flux_above_array_long)
    np.savetxt(results_dir/'newhom_flux_below_array_long_{}'.format(run), hom_flux_below_array_long)
...
def time_sim_full(eps,n_chunk=20,num_steps=48, final_time=12):
    ...
    for i in range(num_steps):
       ...
       ########FULL SIMULATION CODE
        ...
        full_flux_above_array_long[n]=average_flux_above_squish
        full_flux_below_array_long[n]=average_flux_below_squish
    ...
    ###save as txt files to call into plotting code
    np.savetxt(results_dir/'times_list_long_{}'.format(run),times_list_roundlongarray)
    np.savetxt(results_dir/'newfull_flux_above_array_long_{}'.format(run), full_flux_above_array_long)
    np.savetxt(results_dir/'newfull_flux_below_array_long_{}'.format(run), full_flux_below_array_long)
...
def time_sim_em(eps,n_chunk=20,num_steps=12, final_time=3):
    ...
    u_EM1_array=np.empty(num_steps+1,dtype =object)
    u_EM1_array[0] =u_EM1_init.x.array.copy() 
    u_EM2_array=np.empty(num_steps+1,dtype =object)
    u_EM2_array[0]=u_EM2_init.x.array.copy()
    du_EM1 = Function(V_nm_t)
    du_EM2 = Function(V_nm_t)
    du_EM1.x.array[:] = 0.0
    du_EM2.x.array[:] = 0.0
    du_EM1_array=np.empty(num_steps+1,dtype =object)
    du_EM2_array=np.empty(num_steps+1,dtype =object)
    du_EM1_array[0]=du_EM1.x.array.copy()
    du_EM2_array[0]=du_EM2.x.array.copy()
    ...
    hem_array = np.empty(num_steps+2, dtype = object) ####check how big it needs to be
    fem_array = np.empty(num_steps+2, dtype = object)

    Hemm2 = Function(V_nm_t)
    Hemm2.x.array[:]= 0.0
    hem_array[0] = Hemm2.x.array.copy()


    Hemm1 = Function(V_nm_t)
    Hemm1.x.array[:]= 0.0
    hem_array[1] = Hemm1.x.array.copy()

    Femm2 = Function(V_nm_t)
    Femm2.x.array[:]= 0.0
    fem_array[0] =  Femm2.x.array.copy()

    Femm1 = Function(V_nm_t)
    Femm1.x.array[:]= 0.0
    fem_array[1] = Femm1.x.array.copy()
    ...
    # def EM_f_term(t,n):
    #     return sqrt(t/pi)*((2*eps*L**2)/(L+chi))*(fem_array[n+2]-fem_array[n+1])

    def EM_f_term(t,n):
        return sqrt(t/pi)*((eps*L**2)/(L+chi))*(fem_array[n+2]-fem_array[n+1])
    
    ##changed from t to t + delt

    def EM_h_term(t,n):
        return sqrt(t/pi)*2*eps*L*(hem_array[n+2] - hem_array[n+1])
    ...
    u_EM_vis.vector[:] = u_EM_1_vis.x.array + u_EM_2_vis.x.array
    ...
    ##to store error and times
    times_list_roundshortarray = np.array([round(i*delt,3) for i in range(1,num_steps+1)])
    EM_flux_above_array = np.empty(num_steps, dtype = object)
    EM_flux_below_array =np.empty(num_steps, dtype = object)
    ...
    for i in range(num_steps):
        ...
        hemn.x.array[:]= (u_EM2_array[n]+ u_EM1_array[n])/L - chi/(2*eps*L)*(du_EM2_array[n] - du_EM1_array[n])
        femn= Function(V_nm_t)
        femn.x.array[:] = (u_EM2_array[n] - u_EM1_array[n])/L - chi/(2*eps*L)*(du_EM2_array[n] + du_EM1_array[n])
        hem_array[n+2]=hemn.x.array.copy()
        fem_array[n+2] =femn.x.array.copy()
        
        EMh_val= EM_h_term(t,n)[:]
        EMf_val = EM_f_term(t,n)[:] 
        ...
        du_EM_1_sol.x.array[:] = eps/(L+chi)*(u2_EM_vis_array - u1_EM_vis_array) -EMh_val + EMf_val
        du_EM_2_sol.x.array[:] = eps/(L+chi)*(u2_EM_vis_array - u1_EM_vis_array) +EMh_val + EMf_val

        #EM
        u_EM1_array[n+1]=u_EM_1_vis.x.array.copy()
        u_EM2_array[n+1]= u_EM_2_vis.x.array.copy()
        du_EM1_array[n+1]=du_EM_1_sol.x.array.copy()
        du_EM2_array[n+1] =du_EM_2_sol.x.array.copy()
        ...
        EM_flux_above_array[n] =average_flux_above_EM
        EM_flux_below_array[n] =average_flux_below_EM
    ...
    ###save as txt files to call into plotting code
    np.savetxt(results_dir/"times_list_short_{}".format(run),times_list_roundshortarray)
    np.savetxt(results_dir/'newEM_flux_above_array_{}'.format(run), EM_flux_above_array)
    np.savetxt(results_dir/'newEM_flux_below_array_{}'.format(run), EM_flux_below_array)


eps_list =[0.25,0.05]
for eps in eps_list:
    time_sim_effective(eps,n_chunk=20,num_steps=48,final_time =12)
    time_sim_full(eps,n_chunk=20,num_steps=48,final_time =12)
    time_sim_em(eps,n_chunk=20,num_steps=12,final_time =3)
