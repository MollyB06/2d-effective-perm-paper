...
def effective_sim(delt, final_time = 2.55):
    ...
    #define variational form for effective condition
    ...
    ###so for h' and f' at timestep i we just want h(i)-h(i-1)/deltat
    #jump condition to be applied across interface
    #interface condition constant

    p = Constant(mesh_hom, PETSc.ScalarType(P))
    jmp_u = avg(u2) - avg(u1)
    jmp_v = avg(v2)- avg(v1) #### this was wrong before had v2 - v1 instead of v1-v2
    ...
    for i in range(num_steps):
        ...
        #need to split solution and fix parts of domain before updating scalars
        ...
        #append the solutions that are zero in the right parts of the domain?
...
def full_sim(delt, final_time = 2.55):
    ...
    times_list=[num*delt for num in range(num_steps+1)]
    expvalues =[exp(time) for time in times_list]
    ######################      MESH AND SET UP FOR FULL SIMULATION   #############
    ...
    dx_full = Measure("dx", domain = mesh_full, subdomain_data= ct_full)
    ...
    a_full = u * v * dx_full+ delt * ufl.dot(ufl.grad(u), ufl.grad(v)) * dx_full
    l_full = (u_n_full + delt * f_full) * v *dx_full
    ...
    for i in range(num_steps):
        ...
        xdmf_full.write_function(uh_full, t)
